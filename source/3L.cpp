...
vector<Matrix> alg3L(vector<double> vx, vector<double> vy)
{
	...
	quickSort(vec, 0, vec.size() - 1, tmp);
...
void levels(vector_pairs points, double d, vector_pairs& l1, vector_pairs& l3) {
	...
	auto name = dir + to_string(1) + type;
...
void solve_system(Matrix& M, double d) {
	// Решает СЛАУ с помощью псевдообратной матрицы - находит коэффициенты многочлена и записывает их в файл "coef.txt"
